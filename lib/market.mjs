...
export function calculateSevenDayChange(ohlcvList) {
  if (!Array.isArray(ohlcvList) || ohlcvList.length < 8) return null;
  const newestClose = toNumber(ohlcvList[0]?.[4]);
  const oldestClose = toNumber(ohlcvList[7]?.[4]);
  if (newestClose === null || oldestClose === null || oldestClose === 0) return null;
...
export function normalizePool(
  ...
  return {
    ...
    fdv: {
      usd: fdvUsd,
      ada: convertUsdToAda(fdvUsd, priceUsd, derivedPriceAda)
    },
...
export function fallbackToken(token, previousToken, adaUsd = null) {
  ...
  return {
    ...
    change: { h1: null, h24: null, d7: null },
